$PBExportHeader$n_payments.sru
forward
global type n_payments from n_bll
end type
end forward
...
string is_result_dataobject = "d_search_payments"
string is_master_dataobject = "d_payment"
string is_ar_invoice_dataobject = "d_inv_contract"
string is_en_invoice_dataobject = "d_inv_contract"
string is_invoice_btn_title = "طباعة"
end type
global n_payments n_payments
...
IF(dwo.name = "b_clients")Then
	...
	IF( upperbound(lstr_parm.string_parm) > 0 )Then
		...
		IF IsNull(ll_id) THEN ll_id = 0
     	...
     	idw_master.SetItem( row , "account" , lstr_parm.long_array[1] - ll_id )
...
public function integer of_master_validation (long row);Long ll_account ,ll_shares , ll_shares_old
...
IF ( IsNull(ll_account) or ll_account = 0 ) and IsNull(ls_id) THEN
...
lstr_parm.String_parm[1] = "d_inv_contract"
...
on n_payments.create
call super::create
end on

on n_payments.destroy
call super::destroy
end on
