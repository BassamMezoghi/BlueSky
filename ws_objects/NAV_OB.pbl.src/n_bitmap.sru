...
FUNCTION Long GlobalAlloc (Long wFlags, Long dwBytes)  LIBRARY "kernel32"
FUNCTION Long GlobalLock (Long hMem) LIBRARY "kernel32"
FUNCTION Long GlobalUnlock (Long hMem) LIBRARY "kernel32"
Subroutine CopyMemory ( ref blob Destination, long Source, Long Length) Library "kernel32" Alias For "RtlMoveMemory"
...
public subroutine of_copy (blob ab_blob)
end prototypes
...
public subroutine of_copy (blob ab_blob);oleObject ole

ole = Create OleObject

ole.ObjectData = ab_blob

ole.copy()


//long GlobalHandle,ll_hWnd
//long MemoryBlockAddress
//constant int GMEM_MOVEABLE = 2
//blob lbl_tmp
//// get handle to windows background
//ll_hWnd = GetDesktopWindow()
//
////Allocate space for the handle's data and copy the source data into it
//GlobalHandle = GlobalAlloc(GMEM_MOVEABLE, Len(ab_blob) + 1)
//MemoryBlockAddress = GlobalLock(GlobalHandle)
//CopyMemory(lbl_tmp , MemoryBlockAddress , Len(ab_blob) + 1 )
//
//
//		If OpenClipboard(ll_hWnd) Then
//			EmptyClipboard()
//			SetClipboardData(CF_BITMAP, MemoryBlockAddress)
//			CloseClipboard()
//		Else
//			MessageBox("OpenClipboard Failed", of_GetLastError())
//		End If
